...
__declspec(dllexport) int dd_drawtext(int sx,int sy,unsigned short int color,int flags,const char *text) {
    ...
    if (flags&DD_SHADE) dd_drawtext(sx-1,sy-1,IRGB(0,0,0),DD_LEFT|(flags&(DD_SMALL|DD_BIG|DD_CENTER|DD_RIGHT|DD_NOCACHE))|DD__SHADEFONT,text);
        ...
        if (flags&512) dd_drawtext(sx-1,sy-1,IRGB(31,31,31),DD_LEFT|(flags&(DD_SMALL|DD_BIG|DD_CENTER|DD_RIGHT|DD_NOCACHE))|DD__FRAMEFONT,text);
        else dd_drawtext(sx-1,sy-1,IRGB(0,0,0),DD_LEFT|(flags&(DD_SMALL|DD_BIG|DD_CENTER|DD_RIGHT|DD_NOCACHE))|DD__FRAMEFONT,text);
    }
